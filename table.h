...
template<typename in_t, typename out_t, typename UnaryOperation> class basic_unary_col_adder : public pass {
  ...
  struct col_t { size_t col; bool passthrough; std::vector<UnaryOperation> unary_ops; };
...
public:
  ...
  basic_unary_col_adder& add(const char* regex, const char* new_key, const UnaryOperation& unary_op, bool remove_source = 0);
...
public:
  ...
  basic_binary_col_adder& add(const char* regex, const char* other_key, const char* new_key, const BinaryOperation& binary_op, bool remove_source = 0, bool remove_other = 0);
...
template<typename in_t, typename out_t, typename UnaryOperation>
basic_unary_col_adder<in_t, out_t, UnaryOperation>& basic_unary_col_adder<in_t, out_t, UnaryOperation>::add(const char* regex, const char* new_key, const UnaryOperation& unary_op, bool remove_source)
{
...
void basic_unary_col_adder<in_t, out_t, UnaryOperation>::process_token(const char* token, size_t len)
{
  if(first_row) {
    ...
    if(!remove_source) out->process_token(token, len);
  }
  else {
...
template<typename in1_t, typename in2_t, typename out_t, typename BinaryOperation>
basic_binary_col_adder<in1_t, in2_t, out_t, BinaryOperation>& basic_binary_col_adder<in1_t, in2_t, out_t, BinaryOperation>::add(const char* regex, const char* other_key, const char* new_key, const BinaryOperation& binary_op, bool remove_source, bool remove_other)
{
  const char* err; int err_off; pcre* p = pcre_compile(regex, 0, &err, &err_off, 0);
  ...
  if(first_row) {
    ...
    for(typename map<size_t, vector<new_col_info_t> >::iterator i = new_cols.begin(); i != new_cols.end(); ++i) {
      for(typename vector<new_col_info_t>::iterator j = (*i).second.begin(); j != (*i).second.end(); ++j) {
        ...
        out->process_token((*j).new_key.c_str(), (*j).new_key.size());
      }
    }
